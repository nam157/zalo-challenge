...
def MiniFASNetV1(
    embedding_size=128, conv6_kernel=(7, 7), drop_p=0.2, num_classes=3, img_channel=3
):
    return MiniFASNet(
        keep_dict["1.8M"],
        embedding_size,
...
# (80x80) flops: 0.044, params: 0.43
def MiniFASNetV2SE(
    embedding_size=128, conv6_kernel=(7, 7), drop_p=0.75, num_classes=4, img_channel=3
):
    return MiniFASNetSE(
        keep_dict["1.8M_"],
        embedding_size,
...
if __name__ == "__main__":
    ...
    from load_data import transformer

    def get_kernel(height, width):
        kernel_size = ((height + 15) // 16, (width + 15) // 16)
        return kernel_size

    model = MiniFASNetV2(conv6_kernel=get_kernel(80, 80), num_classes=3)
    ...
    img = torch.rand(size=(3, 128, 128))
    img_trans = transformer["train"](img)
