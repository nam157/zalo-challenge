...
from models.utils import MiniFASNetV1, MiniFASNetV1SE, MiniFASNetV2, MiniFASNetV2SE
...
class MultiFTNet(nn.Module):
    def __init__(
        self, img_channel=3, num_classes=2, embedding_size=128, conv6_kernel=(5, 5)
    ):
...
if __name__ == "__main__":
    ...
    model = MultiFTNet(num_classes=2, conv6_kernel=get_kernel(80, 80))
    img = torch.randn(2, 3, 80, 80)
    print(img.shape)
    pred = model(img)
    print(pred[0].shape)
    # print(model)
